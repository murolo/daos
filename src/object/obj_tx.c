...
dc_tx_cleanup_one(struct dc_tx *tx, struct daos_cpd_sub_req *dcsr)
{
	...
	case DAOS_OBJ_RPC_UPDATE: {
		...
		if (dcsr->dcsr_reasb != NULL) {
			obj_reasb_req_fini(dcsr->dcsr_reasb, dcsr->dcsr_nr);
...
dc_tx_commit_cb(tse_task_t *task, void *data)
{
	...
	D_ASSERTF(tx->tx_pm_ver >= oco->oco_map_version,
		  "Invalid pm version: TX ver %u, RPC ver %u\n",
		  tx->tx_pm_ver, oco->oco_map_version);
...
dx_tx_bulk_prepare(struct daos_cpd_sub_req *dcsr, tse_task_t *task)
{
	...
	rc = obj_bulk_prep(dcu->dcu_sgls, dcsr->dcsr_nr, true, CRT_BULK_RO,
			   task, &bulks);
	if (rc != 0) {
		/* Take over dcsr->dcsr_sgls. */
		dcsr->dcsr_sgls = dcu->dcu_sgls;
		/* That overwrites dcsr->dcsr_sgls. */
		dcu->dcu_bulks = bulks;
		      ...
		      struct daos_csummer *csummer)
{
	...
	if (daos_oclass_is_ec(obj->cob_md.omd_id, &oca)) {
		...
		rc = obj_ec_req_reasb(dcu->dcu_iods->oia_iods, dcu->dcu_sgls,
	...
	if (daos_csummer_initialized(csummer)) {
		...
		rc = daos_csummer_calc_iods(csummer, dcu->dcu_sgls,
					    dcu->dcu_iods->oia_iods, NULL,
					    dcsr->dcsr_nr, true, singv_los, -1,
					    &dcu->dcu_iods->oia_iod_csums);
	}

out:
	if (rc == 0)
		rc = daos_sgls_packed_size(dcu->dcu_sgls, dcsr->dcsr_nr, NULL);
		      ...
		      d_list_t *dtr_list, daos_unit_oid_t *leader_oid)
{
	...
	start = grp_idx * obj_get_replicas(obj);
	...
	for (idx = start + obj->cob_grp_size - 1; idx >= start; idx--) {
		...
		dtr->dtr_group.drg_ids[idx - start] = shard->do_target_id;
		obj_shard_close(shard);
		handled++;
	}
	...
	if (dcsr->dcsr_opc == DCSO_UPDATE) {
		...
		if (dcu->dcu_flags & ORF_EC && reasb_req->tgt_bitmap != NULL) {
			...
			for (idx = 0, j = 0; idx < obj->cob_grp_size; idx++) {
				if (isset(reasb_req->tgt_bitmap, idx))
					dcu->dcu_ec_tgts[j++] = idx;
			}

			dcu->dcu_start_shard = start;
			dcsr->dcsr_ec_tgt_nr = j;
		}
	}

	dtr->dtr_group.drg_tgt_cnt = obj->cob_grp_size;
	...
	if (oca->ca_resil == DAOS_RES_EC && !all) {
		...
		dtr->dtr_group.drg_redundancy = obj->cob_grp_size;
	}
...
dc_tx_commit_prepare(struct dc_tx *tx, tse_task_t *task)
{
	...
	for (i = 0; i < req_cnt; i++) {
		struct daos_cpd_sub_req	*dcsr = &tx->tx_req_cache[i];
		struct dc_object	*obj = dcsr->dcsr_obj;
