...
ds_obj_cpd_clone_reqs(struct dtx_leader_handle *dlh, struct daos_shard_tgt *tgt,
		      ...
		      struct daos_cpd_sub_req *dcsr_parent, int count,
		      ...
		      struct daos_cpd_sub_req **p_dcsr)
{
	...
	for (i = 0; i < count; i++) {
		...
		idx = dcde_parent->dcde_reqs[i].dcri_req_idx;
		...
		if (dcsr_parent[idx].dcsr_opc == DCSO_UPDATE &&
			...
			dcu = &dcsr_parent[idx].dcsr_update;
			split = dcu->dcu_ec_split_req;
			oiod = obj_ec_tgt_oiod_get(split->osr_tgt_oiods,
						   dlh->dlh_sub_cnt + 1,
						   tgt->st_shard);
			D_ASSERT(oiod != NULL);

			dcu->dcu_iods->oia_oiods = oiod->oto_oiods;
			dcu->dcu_iods->oia_oiod_nr = oiod->oto_iod_nr;
			dcu->dcu_iods->oia_offs = oiod->oto_offs;
		}

		dcde->dcde_reqs[i].dcri_shard_idx =
			dcde_parent->dcde_reqs[i].dcri_shard_idx;
		    ...
		    dtx_sub_comp_cb_t comp_cb)
{
	...
	struct dtx_sub_status		*sub;
	crt_endpoint_t			 tgt_ep;
	struct daos_cpd_sg		 head_dcs;
	struct daos_cpd_sg		 dcsr_dcs;
	struct daos_cpd_sg		 dcde_dcs;
	int				 count;
	int				 rc = 0;
	...
	/* IDX[0] is for leader. */
	dcsh = ds_obj_cpd_get_dcsh(dca->dca_rpc, idx + 1);
	dcde = ds_obj_cpd_get_dcde(dca->dca_rpc, idx + 1);
	dcsr_parent = ds_obj_cpd_get_dcsr(dca->dca_rpc, idx + 1);
	...
	if (count < ds_obj_cpd_get_dcde_cnt(dca->dca_rpc, idx + 1) ||
	    exec_arg->flags & DRF_HAS_EC_SPLIT) {
		rc = ds_obj_cpd_clone_reqs(dlh, shard_tgt, dcde_parent,
					   dcsr_parent, count, &dcde, &dcsr);
		if (rc != 0)
			goto out;

		remote_arg->sub_reqs = dcsr;
		remote_arg->req_desc = dcde;
	} else {
		dcsr = dcsr_parent;
		dcde = dcde_parent;
	}

	dcsr_dcs.dcs_type = DCST_REQ;
	dcsr_dcs.dcs_nr = count;
	dcsr_dcs.dcs_buf = dcsr_parent;
	oci->oci_sub_reqs.ca_arrays = &dcsr_dcs;
	oci->oci_sub_reqs.ca_count = 1;

	dcde_dcs.dcs_type = DCST_DISP;
	dcde_dcs.dcs_nr = 1;
	dcde_dcs.dcs_buf = dcde_parent;
	oci->oci_disp_ents.ca_arrays = &dcde_dcs;
	oci->oci_disp_ents.ca_count = 1;

	D_DEBUG(DB_TRACE, "Forwarding CPD RPC to rank:%d tag:%d for DXT "
		DF_DTI"\n",
		tgt_ep.ep_rank, tgt_ep.ep_tag, DP_DTI(&dcsh->dcsh_xid));

	rc = crt_req_send(req, shard_cpd_req_cb, remote_arg);

	D_CDEBUG(rc != 0, DLOG_ERR, DB_TRACE,
		 "Forwarded CPD RPC to rank:%d tag:%d for DXT "
