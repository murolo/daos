...
static inline bool
obj_ec_is_valid_tgt(uint32_t *tgt_map, int *pos, int idx, uint32_t map_size)
{
	int	i;

	for (i = *pos; i < map_size; i++) {
		if (tgt_map[i] == idx) {
			*pos = ++i;
			return true;
		}
		    ...
		    struct obj_ec_split_req **split_req)
{
	...
	for (i = 0; i < tgt_nr; i++) {
		if (tgt_map != NULL &&
		    !obj_ec_is_valid_tgt(tgt_map, &pos, i, map_size))
			continue;
	...
	tgt_oiod = obj_ec_tgt_oiod_get(tgt_oiods, count + 1, tgt_max_idx);
