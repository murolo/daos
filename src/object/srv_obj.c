...
static inline void
ds_obj_cpd_set_sub_result(struct obj_cpd_out *oco, int idx, int result)
{
	int	*ret;

	ret = (int *)oco->oco_sub_rets.ca_arrays + idx;
	*ret = result;
}
		      ...
		      struct obj_io_context *ioc, struct dtx_handle *dth)
{
	...
	for (i = 0; i < dcde->dcde_read_cnt; i++) {
		...
		rc = vos_fetch_begin(ioc->ioc_coc->sc_hdl, dcsr->dcsr_oid,
				     dcsh->dcsh_epoch, VOS_OF_USE_TIMESTAMPS,
				     &dcsr->dcsr_dkey, dcsr->dcsr_nr,
				     dcsr->dcsr_read.dcr_iods,
				     VOS_FETCH_SET_TS_ONLY,
				     NULL, &ioh, dth);
		if (rc == 0)
	...
	for (i = 0; i < dcde->dcde_write_cnt; i++) {
		...
		rc = vos_update_begin(ioc->ioc_coc->sc_hdl,
				...
				dcsr->dcsr_api_flags | VOS_OF_USE_TIMESTAMPS,
				&dcsr->dcsr_dkey,  dcsr->dcsr_nr,
				iods, csums, &iohs[i], dth);
		if (rc != 0)
			goto out;
...
ds_obj_dtx_follower(crt_rpc_t *rpc, struct obj_io_context *ioc)
{
	...
	struct daos_cpd_disp_ent	*dcde = ds_obj_cpd_get_dcde(rpc, 0);
		   ...
		   dtx_sub_comp_cb_t comp_cb)
{
	...
	if (idx == -1) {
		if (!(exec_arg->flags & ORF_RESEND)) {
			 ...
			 dcde = ds_obj_cpd_get_dcde(dca->dca_rpc, 0);
			 ...
			 */
			 ...
			 rc = ds_obj_dtx_handle_one(dca->dca_rpc,
					 ds_obj_cpd_get_dcsh(dca->dca_rpc, 0),
					 dcde,
					 ds_obj_cpd_get_dcsr(dca->dca_rpc, 0),
...
ds_obj_dtx_leader_ult(void *arg)
{
	...
	dcde = ds_obj_cpd_get_dcde(dca->dca_rpc, dca->dca_idx);
